...
export class AuthController {
  ...
  async login(
    ...
    response.cookie(
      'toheyo_session',
      CryptoJS.AES.encrypt(refreshToken, process.env.JWT_SECRET!).toString(),
      { httpOnly: true },
    );
  ...
  async reassignDevice(
    @Request() req: any,
    @Headers('device-hash') deviceHash: string,
  ) {
  ...
  async loginAdmin(
    ...
    const refreshToken = await this.authService.generateAdminRefreshToken(
      req.user,
    );
    response.cookie(
      'toheyo_admin_session',
